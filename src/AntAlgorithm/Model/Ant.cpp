...
double Ant::calculateNumerator(std::map<int, double>* dictionaryOfAvailablePaths,
...
double Ant::calcDenominator(std::map<int, double>* dictionaryOfAvailablePaths,
                            const SquareMatrix& pheromonesMap) {
    double sumOfAllProbabilities = 0.0;
    for (auto& i : (*dictionaryOfAvailablePaths)) {
        sumOfAllProbabilities += pow(1 / i.second, beta) * pow(pheromonesMap(path.back(), i.first), alpha);
    }
